...
class Permission extends Model
{
    ...
    public function getPermission($filters = array(), $returnAssoc = false, $returnSingleRow = false)
    {
    ...
    public function getPermissionForGroup()
    {
        ...
        foreach ($query as $key => $permission) {
            $restrictionQuery = $this->db->table('restrictions')->select('id,permission_id,name,slug')->where('permission_id', $permission['id'])->get()->getResultArray();
            $query[$key]['restrictions'] = $restrictionQuery;
        }
        return $query;
    }
}
