...
class Role extends Model
{
    ...
    protected $allowedFields    = ['name', 'group_id', 'level_id', 'can_delete', 'status', 'created_at', 'updated_at'];
    ...
    public function getRoles($filters = array(), $returnAssoc = false, $returnSingleRow = false)
    {
        // $builder = $this->builder();
        $builder = $this->db->table('role');
        $builder->select('role.*,role_levels.level_name as level_name,permission_group.name as group_name');
        $builder->join('role_levels', 'role_levels.id = role.level_id', 'LEFT');
        $builder->join('permission_group', 'permission_group.id = role.group_id', 'LEFT');
        
        if (isset($filters['id']) && $filters['id'] != '') $builder->where('role.id', $filters['id']);
        ...
        $builder =$builder->whereNotIn('role.id', [1]);
        $query = $builder->get();
