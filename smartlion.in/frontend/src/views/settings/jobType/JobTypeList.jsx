...
import { deleteJobTypeApi, JobTypeListApi } from 'apis/Setting';
...
const JobTypeList = ({ search, callApi }) => {
    ...
    const deleteHandler = () => {
        deleteJobTypeApi({ id: jobTypeData.id })
            .then((res) => {
    ...
    return (
        <>
                        ...
                        {data.map((item, Index) => (
                            <TableRow key={Index}>
                                ...
                                <TableCell align="right">
                                    {checkRestriction('CAN_EDIT_JOB_TYPE') && (
                                        <IconButton color="primary" component="label" onClick={() => editData(item)}>
                                            <EditIcon fontSize="small" />
                                        </IconButton>
                                    )}
                                    {checkRestriction('CAN_DELETE_JOB_TYPE') && (
            ...
            {openEdit && (
                <CenterDialog title="Edit Job Type" open={openEdit} onClose={() => setOpenEdit((prevState) => !prevState)} id="editJobType">
                    <AddEditJobType value={jobTypeData} formID="editJobType" onSubmit={submitHandler} />
                </CenterDialog>
            )}
            {openDelete && (
                <DeleteDialog
