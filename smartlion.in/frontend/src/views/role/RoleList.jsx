...
let recordsTotal = 0;
const RoleList = ({ search, callApi, setInitList, initList }) => {
    ...
    const getData = () => {
        roleListApi(params)
            .then((res) => {
                ...
                setInitList({
                    roleLevelData: res.data.data.levels,
                    permissionGroupData: res.data.data.permissionGroups
                });
            })
            .catch((err) => {
    ...
    return (
        <>
            ...
            {openEdit && (
                <CommonDialog open={openEdit} title="Edit Role" onClose={() => setOpenEdit((prevState) => !prevState)} id="editRole">
                    <AddEditRole RoleValue={roleData} formId="editRole" onSubmit={submitHandler} initList={initList} />
                </CommonDialog>
            )}
