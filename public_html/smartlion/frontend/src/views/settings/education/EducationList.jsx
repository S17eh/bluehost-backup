...
import { deleteEducationApi, EducationListApi } from 'apis/Setting';
...
const EducationList = ({ search, callApi, initData, setInitData }) => {
    ...
    const deleteData = (row) => {
        setOpenDelete(true);
        setEducationData(row);
    };

    const deleteHandler = () => {
        deleteEducationApi({ id: educationData.id })
            .then((res) => {
        ...
        <>
                        ...
                        {data.map((item, Index) => (
                            <TableRow key={Index}>
                                ...
                                <TableCell align="right">
                                    {checkRestriction('CAN_EDIT_EDUCATION') && (
                                        <IconButton color="primary" component="label" onClick={() => editData(item)}>
                                            <EditIcon fontSize="small" />
                                        </IconButton>
                                    )}
                                    {checkRestriction('CAN_DELETE_EDUCATION') && (
            ...
            {openEdit && (
                <CommonDialog
                    ...
                    id="editEducation"
                >
                    <AddEditEducation value={educationData} formId="editEducation" onSubmit={submitHandler} initData={initData} />
                </CommonDialog>
            )}
    ...
    return (
        <TableHead>
            <TableRow>
                ...
                <TableCell key="type" sx={{ width: '30%' }}>
                    <TableSortLabel active={orderBy === '0'} direction={orderBy === '0' ? order : 'asc'} onClick={createSortHandler('0')}>
                        Type
                    </TableSortLabel>
                </TableCell>
