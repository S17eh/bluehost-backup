...
import AddEditFunctionalArea from './AddEditFunctionalArea';
import { deleteFunctionalAreaApi, FunctionalAreaListApi } from 'apis/Setting';
...
const FunctionalAreaList = ({ search, callApi }) => {
    ...
    const deleteData = (row) => {
        setOpenDelete(true);
        setFunctionalAreaData(row);
    };

    const deleteHandler = () => {
        deleteFunctionalAreaApi({ id: FunctionalAreaData.id })
            .then((res) => {
        ...
        <>
                        ...
                        {data.map((item, Index) => (
                            <TableRow key={Index}>
                                ...
                                <TableCell align="right">
                                    {checkRestriction('CAN_EDIT_FUNCTIONAL_AREA') && (
                                        <IconButton color="primary" component="label" onClick={() => editData(item)}>
                                            <EditIcon fontSize="small" />
                                        </IconButton>
                                    )}
                                    {checkRestriction('CAN_DELETE_FUNCTIONAL_AREA') && (
            ...
            {openEdit && (
                <CenterDialog
                    ...
                    id="editFunctionalArea"
                >
                    <AddEditFunctionalArea value={FunctionalAreaData} formID="editFunctionalArea" onSubmit={submitHandler} />
                </CenterDialog>
            )}
            {openDelete && (
                <DeleteDialog
